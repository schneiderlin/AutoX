...
- **WHEN** server sends `{:event "get-layout"}`
- **AND** accessibility service is enabled
- **THEN** client SHALL capture current window using `LayoutInspector`
- **AND** client SHALL respond with `{:event "layout" :data {...}}` containing serialized NodeInfo tree
...
- **THEN** client SHALL respond with error `{:event "error" :message "Accessibility service not enabled"}`
...
- **THEN** response SHALL include `{:event "layout" :data nil}` or error message
...
- **WHEN** server receives `{:event "layout" :data layout}`
...
- **THEN** server SHALL send `{:event "get-layout"}` to specified client
- **AND** server SHALL expect `:layout` response from that client

### Requirement: Bidirectional Communication Pattern
The WebSocket protocol SHALL support both client-initiated and server-initiated messages using `event` field.
...
- **THEN** client SHALL process unknown commands with appropriate error handling
